...
export class FeishuAdapter implements IMAdapter {
  ...
  async sendReply(
    ...
    // If there's a thinking message to delete, remove it first
    const thinkingMsgId = replyContext?.feishuThinkingMessageId as string | undefined;
    if (thinkingMsgId) {
      await this.deleteMessage(appId, appSecret, domain, thinkingMsgId);
    }
          ...
          try {
            ...
            if (messageType === 'text') {
              ...
              try {
                ...
                text = `[用户发送了文件: ${fileName}，请帮我处理这个文件]`;
